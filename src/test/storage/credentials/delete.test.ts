...
suite('Storage:Credentials:Delete', () => {
    ...
    test('deletes an existing credential by username', async () => {
        ...
        mockContext.globalState.update = async (key: string, value: string) => {
            calls.update.push([key, value])
        }

        mockContext.secrets.delete = async (key: string) => {
        ...
        // Act
        await Storage.credential.delete(mockContext, usernameToDelete)
